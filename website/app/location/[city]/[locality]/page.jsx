...
export async function generateMetadata({ params }) {
  ...
  return {
    title: `Properties in ${locality}, ${city}`,
    description: `Browse active property listings in ${locality}, ${city}.`,
    alternates: { canonical: `${siteUrl()}/location/${params.city}/${params.locality}` },
  };
}
...
export default async function LocalityPage({ params }) {
  const city = decodeURIComponent(params.city);
  const locality = decodeURIComponent(params.locality);
  const data = await api(`/api/website/properties?city=${encodeURIComponent(city)}&locality=${encodeURIComponent(locality)}&limit=20`);

  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8">
      <h1 className="mb-4 text-2xl font-bold">Properties in {locality}, {city}</h1>
